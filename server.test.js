...
const { app, validateHTML, getServerUptime, getServerStatus } = require('./server');
...
describe('Tests Unitarios - Funciones del servidor', () => {
  ...
  test('validateHTML debe fallar si falta DOCTYPE', () => {
    ...
    expect(() => validateHTML(htmlInvalido)).toThrow('HTML inválido: falta la etiqueta <!DOCTYPE html>');
  });

  test('validateHTML debe fallar si falta etiqueta html', () => {
    const htmlInvalido = '<!DOCTYPE html><head></head><body></body>';
    expect(() => validateHTML(htmlInvalido)).toThrow('HTML inválido: falta la etiqueta <html');
  });
